...
- 표준 샘플 계정(비번 공통 `rhdxhd12`, 대소문자 구분): 업무사용자 `TEST1`·`webmaster`, 일반회원 `USER`, 기업회원 `ENTERPRISE`, 관리자 `admin`/`1`. 상세는 [`로그인-테스트계정-가이드.md`](로그인-테스트계정-가이드.md) 참고
- 예제는 로그인 없이(익명) 실제 DB와 연동되어 동작
...
## SQL 작성 규칙(코드 컨벤션)
전자정부 표준프레임워크 SQL 작성 규칙(대문자 키워드·우측정렬·앞쪽 콤마·AS 별칭·`SELECT *` 금지·`/* */` 주석·바인드변수)은 [`sql-코드컨벤션.md`](sql-코드컨벤션.md) 참조. 신규/수정 쿼리는 이 규칙을 따른다.
